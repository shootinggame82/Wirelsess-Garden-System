...
<head>
    <title><?php echo $sys_name; ?> - <?=gettext('Odlings loggar')?></title>
  ...
  <?php include 'include/header.php'; ?>
                              ...
                              <div class="row align-items-center">
                                  <div class="col-md-8">
                                      <div class="page-header-title">
                                          <h4 class="m-b-10"><?=gettext('Odlings loggar')?></h4>
                                      </div>
                                      <ul class="breadcrumb">
                                          ...
                                          <li class="breadcrumb-item">
                                              <a href="#!"><?=gettext('Odlings loggar')?></a>
                                          </li>
                                      </ul>
                                  </div>
                              </div>
                                    ...
                                    <div class="page-body">
                                        <div class="card">
                                            <div class="card-header">
                                                <h5><?php echo $azonnamn; ?> <?=gettext('odlings loggar')?></h5>
                                                <span><?=gettext('Här finner du alla loggar du har skapat för din odling. Klicka på namnet för att kunna öppna och redigera loggen.')?></span>
                                            </div>
                                                ...
                                                <div class="table-responsive">
                                                    <table class="table">
                                                        <thead>
                                                            <tr>
                                                                <th><?=gettext('Namn')?></th>
                                                                <th><?=gettext('Senast ändrad')?></th>
                                                                <th><?=gettext('Meny')?></th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                          <?php    $sql = "SELECT * FROM loggbok WHERE zon = '$idet' ORDER BY id DESC";
                                                              $result = $mysqli->query($sql);

                                                              if ($result->num_rows > 0) {
                                                                  while ($row = $result->fetch_assoc()) {
                                                                      $loggrubrik = $row["rubrik"];


                                                                      $loggred = $row["redigerad"];
                                                                      $loggid = $row["id"]; ?>
                                                            <tr>
                                                                <td><a href="zonloggedit.php?id=<?php echo $loggid; ?>"><?php echo $loggrubrik; ?></td>
                                                                <td><?php echo $loggred; ?></td>
                                                                <td><a href="#!" onclick="radera(<?php echo $loggid; ?>, <?php echo $idet; ?>)"><?=gettext('Radera')?></td>
                                                            </tr>
                                                            <?php
                                                                  }
                                                              } ?>
    ...
    <script type="text/javascript" src="assets/js/zonsensorer.js.php"></script>
    <script type="text/javascript" src="assets/js/zonloggar.js.php"></script>
</body>

...
<head>
    <title><?php echo $sys_name; ?> - <?=gettext('Zon regnsensorer')?></title>
  ...
  <?php include 'include/header.php'; ?>
                              ...
                              <div class="row align-items-center">
                                  <div class="col-md-8">
                                      <div class="page-header-title">
                                          <h4 class="m-b-10"><?=gettext('Zon regnsensorer')?></h4>
                                      </div>
                                      <ul class="breadcrumb">
                                    ...
                                    <div class="page-body">
                                        <div class="card">
                                            <div class="card-header">
                                                <h5><?php echo $azonnamn; ?> <?=gettext('regnsensorer')?></h5>
                                                <span><?=gettext('Här ser du lista på dina regnsensorer som är kopplade till denna zon. Genom att klicka på radera så tar du bort sensorn från zonen (ej från systemet)')?></span>
                                            </div>
                                                    ...
                                                    <table class="table">
                                                        <thead>
                                                            <tr>
                                                                <th><?=gettext('Nr')?></th>
                                                                <th><?=gettext('Namn')?></th>
                                                                <th><?=gettext('Regnfuktighet')?></th>
                                                                <th><?=gettext('Regnat nyligen')?></th>
                                                                <th><?=gettext('Volt')?></th>
                                                          ...
                                                          <?php    $sql = "SELECT * FROM zonregn WHERE zon = '$idet' ORDER BY id ASC";
                                                              ...
                                                              if ($result->num_rows > 0) {
                                                                  while ($row = $result->fetch_assoc()) {
                                                                      ...
                                                                      $sql2 = "SELECT * FROM regnsensor WHERE id = '$ensensor'";
                                                                      $result2 = $mysqli->query($sql2);

                                                                      if ($result2->num_rows == 1) {
                                                                          while ($row2 = $result2->fetch_assoc()) {
                                                                              $ensensornr = $row2["regnid"];
                                                                              $ensensornamn = $row2["namn"];
                                                                              $sql3 = "SELECT * FROM regndata WHERE regnnr = '$ensensornr' ORDER BY id DESC LIMIT 1";
                                                                              $result3 = $mysqli->query($sql3);

                                                                              if ($result3->num_rows > 0) {
                                                                                  while ($row3 = $result3->fetch_assoc()) {
                                                                                      $ensensorfukt = $row3["regnfukt"];
                                                                                      $ensensorjord = $row3["regnar"];
                                                                                      $ensensorvolt = $row3["volt"]; ?>
                                                            <tr>
                                                                <th scope="row"><?php echo $ensensornr; ?></th>
                                                                <td><?php echo $ensensornamn; ?></td>
                                                                <td><?php echo $ensensorfukt; ?></td>
                                                                <?php if ($ensensorjord == 1) {
                                                                                          ?>
                                                                  <td><?=gettext('Ja')?></td>
                                                            ...
                                                            <?php
                                                                ...
                                                                <td><a href="#!" onclick="radera(<?php echo $ensensor; ?>, <?php echo $idet; ?>)"><?=gettext('Radera')?></A></td>
                                                            </tr>
                                                            <?php
                                                                                  }
                                                                              }
    ...
    <script type="text/javascript" src="assets/js/zonregnar.js.php"></script>
</body>

...
<head>
    <title><?php echo $sys_name; ?> - <?=gettext('Redigera sensorer')?></title>
  ...
  <?php include 'include/header.php'; ?>
                              ...
                              <div class="row align-items-center">
                                  <div class="col-md-8">
                                      <div class="page-header-title">
                                          <h4 class="m-b-10"><?=gettext('Redigera Jord sensorer')?></h4>
                                      </div>
                                      <ul class="breadcrumb">
                                    ...
                                    <div class="page-body">
                                        <div class="card">
                                            <div class="card-header">
                                                <h5><?=gettext('Redigera jord sensorer')?></h5>
                                                <span><?=gettext('Här ser du lista på dina sensorer som finns i systemet. Du kan ändra dess värden här eller tabort dom (dom försvinner även från zoner då.)')?>'</span>
                                            </div>
                                                    ...
                                                    <table class="table">
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                <th><?=gettext('Autostart')?></th>
                                                                <th><?=gettext('Meny')?></th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                          <?php    $sql = "SELECT * FROM sensorer";
                                                              $result = $mysqli->query($sql);
                                                              ...
                                                              if ($result->num_rows > 0) {
                                                                  while ($row = $result->fetch_assoc()) {
                                                                      ...
                                                                      $sensnamn = $row["namn"];
                                                                      $senspump = $row["pump"];
                                                                      $sensfukt = $row["fuktighet"];
                                                                      $sensauto = $row["autostart"];
                                                                      $sensid = $row["id"];

                                                                      $sql2 = "SELECT * FROM pumpar WHERE id = '$senspump'";
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td><?php echo $pumpnr; ?> - <?php echo $pumpnamn; ?></td>
                                                                <td><?php echo $sensfukt; ?></td>
                                                                <td><?php if ($sensauto == 1) {
                                                                                  echo gettext('Aktiverat');
                                                                              } else {
                                                                                  echo gettext('Inaktiverat');
                                                                              } ?></td>
    ...
    <script type="text/javascript" src="assets/js/redigerasensor.js.php"></script>
</body>

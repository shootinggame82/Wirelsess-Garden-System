  ...
  $(document).ready(function() {
    $.PostItAll.storageManager(function(e) {
      e.test(function(testOk) {
        if (testOk) {
          //Load previous saved notes
          $.PostItAll.load();
        } else {
          //Disable controls
          $('.navExt').addClass('disabled').click(function(e) {
            e.preventDefault();
            e.stopPropagation();
          });
        }
  ...
  function setWindowSticky() {
    ...
    $('#notes').postitall({
      ...
      posY: 200,
      style: {
        backgroundcolor: '#448aff', //Background color in new postits when randomColor = false

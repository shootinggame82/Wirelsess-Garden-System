...
if ($uppdatera == 1) {
    if (isset($idnumret, $namn)) {
        $sql = "SELECT * FROM vilttimer WHERE id = '$idnumret'";
        ...
        if ($existerar == 1) {
            if ($insert_stmt = $mysqli->prepare("UPDATE vilttimer SET namn = ?, start = ?, slut = ?, mon = ?, tis = ?, ons = ?, tor = ?, fre = ?, lor = ?, son = ?, vilt = ? WHERE id = ?")) {
    ...
    if (isset($pump, $namn)) {
        $sql = "SELECT * FROM vilt WHERE id = '$pump'";
        $result = $mysqli->query($sql);
        ...
        if ($existerar == 1) {
            if ($insert_stmt = $mysqli->prepare("INSERT INTO vilttimer (namn, start, slut, mon, tis, ons, tor, fre, lor, son, vilt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
